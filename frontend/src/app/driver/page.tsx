...
export default function DriverLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success && data.data.user.role === 'driver') {
        setAuth(data.data.user, data.data.token);
        toast.success('Welcome, Driver!');
        router.push('/driver/dashboard');
      } else if (data.success) {
        toast.error('This portal is for drivers only.');
      }
    } catch (err: unknown) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-950 via-blue-900 to-blue-900 flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <div className="w-16 h-16 bg-blue-500 rounded-2xl flex items-center justify-center mx-auto mb-4">
            <Car className="w-8 h-8 text-white" />
          </div>
          <h1 className="text-3xl font-extrabold text-white">Driver Portal</h1>
          <p className="text-blue-300 mt-2">LankaJourney.lk — Driver Login</p>
        </div>
        ...
        <div className="bg-white/10 backdrop-blur rounded-2xl border border-white/20 p-8">
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <button type="submit" disabled={loading}
              ...
              <LogIn className="w-4 h-4" /> {loading ? 'Signing In...' : 'Sign In as Driver'}
            </button>
          </form>
        </div>
        <p className="text-center text-white/40 text-xs mt-4">Not a driver? <a href="/" className="text-blue-300 hover:underline">Go back home</a></p>
      </div>
    </div>
  );
}
