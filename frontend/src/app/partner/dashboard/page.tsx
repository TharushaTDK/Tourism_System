      ...
      <div className="flex gap-2 mb-6 border-b border-gray-100 pb-2">
        {TABS.map((tab) => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            className={`px-4 py-2 text-sm font-medium rounded-t-lg transition-colors ${activeTab === tab ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700'}`}>
            {tab}
          </button>
        ))}
      </div>
              ...
              {listings.map((listing: { id: number; name: string; type?: string; category?: string; rating: number; image_urls?: string[] }) => (
                <div key={listing.id} className="bg-white rounded-xl border border-gray-100 p-4 flex gap-4">
                  ...
                  <div className="flex-1">
                    <h3 className="font-bold text-gray-800 text-sm">{listing.name}</h3>
